...
class Editor extends Component<EditorProps, State> {
    ...
    constructor(props: EditorProps) {
        ...
        this.loadSketchesFromLS = this.loadSketchesFromLS.bind(this);
    ...
    loadSketchesFromLS = () => {
        const lsSprites = (window as any).localStorage.getItem("spriteList");

        if (lsSprites && lsSprites !== []) {
            this.setState({sprites: JSON.parse(lsSprites)})
        }
    ...
    exportBinary = () => {
        ...
        ipcRenderer.send('export', {
            code: this.getCode(),
            path,
            device: this.props.device ? this.props.device : "cm:esp32:ringo",
            minimal: this.state.minimalCompile
        });
    };
    ...
    removeSprites = () => {
        (window as any).localStorage.removeItem("spriteList");
        this.setState({
            sprites: []
        });
    }

    async  saveCells(cells: boolean[], name: string) {
        const spriteDesign: Sprite = {
            design: cells,
            name: name,
            index: this.state.sprites.length
        };
        await this.setState({
            sprites: [...this.state.sprites, spriteDesign],
            nameExists: false
        });
        (window as any).localStorage.removeItem("spriteList");
        (window as any).localStorage.setItem("spriteList", JSON.stringify(this.state.sprites));
    }

    updateSprite(cells: any, index: any, name: string) {
        let array = [...this.state.sprites];
        for (let i = 0; i < this.state.sprites.length; i++) {
            if (array.indexOf(array[i]) === index) {
                if (index !== -1) {
                    array[i].design = cells;
                    array[i].name = name;
                    this.setState({sprites: array});
                    (window as any).localStorage.removeItem("spriteList");
                    (window as any).localStorage.setItem("spriteList", JSON.stringify(this.state.sprites));
                } else {
                    alert("Hey, you shouldn't be able to get here! Contact us!")
                }
            }
        }
    }
    ...
    saveSprites(spriteList: any) {
        (window as any).localStorage.setItem("spriteList", JSON.stringify(spriteList));
        this.setState({isSpriteEditorOpen: false});
    }
            ...
            <div className={isEditorOpen ? 'e-open' : 'e-close'} style={{backgroundColor: '#F9F9F9'}}>
                {isEditorOpen && (
                    <React.Fragment>
                        ...
                        <SpriteEditorModal
                            saveSprites={(spriteList) => this.saveSprites(spriteList)}
                            updateSprite={(cells: any, index: string, name: string) => this.updateSprite(cells, index, name)}
                            deleteSprite={(index) => this.deleteModal(index)}
                            saveCells={(cells: any, name: any) => this.saveCells(cells, name)}
                            open={this.state.isSpriteEditorOpen}
                            closeSpriteModal={() => this.setState({isSpriteEditorOpen: false})}
                            spriteList={this.state.sprites}
                            deleteAllSprites={this.removeSprites}/>
                        <EditorHeader
                            openEditSprite={() => this.setState({isSpriteEditorOpen: true})}
                            home={openHome}
                ...
                {isCodeOpen && isEditorOpen && (
                    <EditorPopup className={(isCodeFull || type == SketchType.CODE) ? 'fullscreen' : ''} theme={theme}>
                        ...
                        <Monaco ref={monacoRef} startCode={startCode} theme={theme} editing={true}/> ||
